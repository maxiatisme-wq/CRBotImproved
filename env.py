...
class ClashRoyaleEnv:
    ...
    def reset(self):
        ...
        threading.Thread(target=self._initialize_max_tower_health, daemon=True).start()
    ...
    def normalize_health(self):
        ...
        if all(i is not None for i in self.max_tower_health) and all(i is not None for i in self.tower_health_values):
            ...
            reliability_flags = np.ones_like(normal_health_values) #when reliability flag = 1 that means the value was read from the ocr
            ...
            for i in range(6):
                ...
                if self.max_tower_health[i] == None or self.tower_health_values[i] == None:
                    normal_health_val_and_flag.append(-1) #-1 means undefined
                    normal_health_val_and_flag.append(0) #when the normal health value is -1 the reliability is 0 because the -1 is neant to be a place holder
                else:
    ...
    def _get_state(self):
        ...
        self.current_predictions = self._get_all_predictions()
        ...
        if not self.current_predictions:
            ...
            zero_state = np.array([elixir / 10.0] + [0.0] * (self.state_size - 1), dtype=np.float32)
            zero_state[-12:] = health_state # Still include health state
            return zero_state
        ...
        state = np.array([elixir / 10.0] + ally_flat + enemy_flat + health_state, dtype=np.float32)
        return state
    ...
    def _compute_reward(self, state):
        ...
        enemy_positions = state[1 + 2 * MAX_ALLIES:-12]  # All enemy x1, y1, x2, y2, ...
    ...
    def _count_ally_princess_towers(self, predictions: List[Dict]) -> int:
        return sum(1 for p in predictions if isinstance(p, dict) and p.get("class") == "ally princess tower")
    ...
    def _get_tower_data(self, predictions: List[Dict], class_name: str) -> Tuple[int, Optional[float]]:
        ...
        for p in predictions:
            if isinstance(p, dict) and p.get("class") == class_name:
